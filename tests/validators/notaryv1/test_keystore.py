...
pub_root_keys = {
    "7c62922e6be165f1ea08252f77410152b9e4ec0d7bf4e69c1cc43f0e6c73da20": Key(
        (
            "MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAErIGdt5pelfWOSjmY7k+/TypV0IFF"
            "9XLA+K4swhclLJb79cLoeBBDqkkUrkfhN5gxRnA//wA3amL4WXkaGsb9zQ=="
        ),
    ),
    "7dbacd611d5933ca3f0fad581ed233881c501229343613f63f2d4b5771ee4299": Key(
        (
    ...
    "f1997e14be3d33c5677282b6a73060d8124f4020f464644e27ab76f703eb6f7e": Key(
        (
...
target_keys = {
    "6984a67934a29955b3f969835c58ee0dd09158f5bec43726d319515b56b0a878": Key(
        (
    ...
    "70aa109003a93131c63499c70dcfc8db3ba33ca81bdd1abcd52c067a8acc0492": Key(
        (
...
def test_key_store(pub_key: Key):
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "root",
            dict(**{"root": Key(sample_key)}, **pub_root_keys),
            {},
        ),
        (
            fix.get_td("sample_targets"),
            "targets",
            dict(**{"root": Key(sample_key)}, **target_keys),
            {},
        ),
        (
